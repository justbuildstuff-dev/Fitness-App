...
void main() {
  ...
  group('Superset Group Operations', () {
    ...
    test('createSuperset calls FirestoreService with shared supersetGroupId', () async {
      ...
      // captured[0] = exercise list, captured[1] = supersetGroupId, captured[2] = setCount
      ...
      // All exercises share the same supersetGroupId
      for (final ex in capturedExercises) {
        expect(ex.supersetGroupId, capturedGroupId);
      }
    });

    test('createSuperset assigns sequential groupOrderIndex to exercises', () async {
    ...
    test('createSuperset returns null when userId is missing', () async {
      ...
      final unauthProvider = ProgramProvider.withServices(
        null, mockFirestoreService, mockAnalyticsService,
      );
      ...
      expect(ids, isNull);
      verifyNever(mockFirestoreService.createSupersetWithExercises(any, any, any));
    });

    test('deleteSupersetGroup calls FirestoreService with correct parameters', () async {
      /// Test Purpose: Verify deleteSupersetGroup passes all IDs to the service
    ...
    test('deleteSupersetGroup rethrows on service error', () async {
      /// Test Purpose: Verify errors propagate so UI can show feedback
      when(mockFirestoreService.deleteSupersetGroup(any, any, any, any, any))
